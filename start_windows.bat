...
set arguments_file=CMD_FLAGS.txt
...
:: call python server.py --listen --cpu --sd-remote --enable-modules=caption,summarize,classify,tts --use-faster-whisper --stt-whisper-model-path="E:\SillyTavern-Launcher-main\SillyTavern-Extras\data\models\faster-whisper" %*

:: Check if the arguments file exists
if not exist "%arguments_file%" (
    call :log_message "ERROR" "Arguments file '%arguments_file%' not found."
    pause
    exit /b 1
)
